...
class OdooApi {
  ...
  _getDatabases() {
    return Promise.resolve([this.hostname]); // Return a promise with the inputted URL as the only database
  }

  async connect(database = null) {
    if (!database) {
      database = this.hostname; // Use the base URL as the database name
    }
